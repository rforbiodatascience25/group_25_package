...
Dependencies include the packages `stringr` and `ggplot2`. Their usage is in function 5 for character handling and visualization. stringr is also used to check for DNA in the codon-to-protein function.
...
Besides some coding differences, the issue lies like with library() and '::' operator. Usage of the `@importFrom package function` might cause some issues if functions from packages used this way have some names. Contrary `package::function()` makes sure that intended function is used. Second option looks like obvious choice, but it also makes code larger and possibly harder to interpret (although it makes it also more straightforward to interpret, so there is some balance here). First option is better choice in some specific use cases, but usually second option would be chosen for certainty.
