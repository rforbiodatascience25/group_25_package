...
link to the group´s GitHub: https://github.com/rforbiodatascience25/group_25_package
...
Gets all codons as separate strings from indicated point in given sequence. The codons are identified based on codon:protein table from 'codo'variable.
...
Dependencies are discussed below. Dependencies must be managed so the code can run in most environments and on most machines with a minimum number of requirements for usage. Fewer dependencies means introducing a smaller number of factors that could make the code produce different outcomes or act differently on different machines.

#### Dependencies of the package

Dependencies include the packages `stringr` and `ggplot2`. Their usage is in function 5 for character handling and visualization. stringr is also used to check for DNA in the codon-to-protein function. Dependencies were added as Imports in the DESCRIPTION file of the package, meaning they are automatically installed for use when the package is called.
...
Besides some coding differences, the issue lies with library() and '::' operator. Usage of the `@importFrom package function` might cause some issues if functions from packages used this way have some names. Contrary `package::function()` makes sure that intended function is used. Second option looks like the obvious choice, but it also makes the code longer and possibly harder to interpret (although it also makes it  more straightforward to interpret, so there is some balance here). The first option is a better choice in some specific use cases, but usually the second option would be chosen for certainty.
